...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, layer_idx: Optional[int] = None, applied_module: str = None, gate_hp={'temperature': 0.33, 'stretch_limits': (-0.1, 1.1), 'eps': 1e-6}):
        super().__init__()
        self.config = config
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        ## Use the normal attention implementation rather than flash attention or sdpa
...
class JoLAModel(Qwen2PreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]
    def __init__(self, config):
        super().__init__(config)
        self.model = Qwen2Model(config)
